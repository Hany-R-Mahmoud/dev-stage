...
export const ProfileCard: React.FC<ProfileCardProps> = ({ profile, language }) => {
  ...
  return (
    <div className="relative overflow-hidden bg-[#F9F8F6] dark:bg-[#141312] border-l border-l-[#D4AF37] border-y border-r border-[#1A1A1A]/15 dark:border-white/15 p-5 sm:p-8 md:p-10 shadow-sm font-sans-luxury text-[#1A1A1A] dark:text-[#F4F2ED] transition-colors duration-300">
      <div className="relative z-10 flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-8">
        
        {/* Avatar with Refined Border & Grayscale to Color Hover Effect */}
        <div className="relative shrink-0 group cursor-pointer">
          <div className="h-28 w-22 sm:h-32 sm:w-24 md:h-36 md:w-28 overflow-hidden border border-[#1A1A1A]/30 dark:border-white/30 shadow-md">
            <img
              src={profile.avatar}
              ...
              style={{ objectPosition: 'center 22%', transform: 'scaleX(-1)' }}
            />
          </div>
        ...
        <div className="flex flex-1 flex-col items-center md:items-start ltr:md:text-left rtl:md:text-right text-center space-y-3">
          ...
          <div className="flex w-full flex-col items-center justify-center gap-2 md:flex-row md:justify-start md:gap-3">
            <h2 className="font-serif-luxury text-[2rem] leading-tight md:text-4xl font-bold tracking-tight text-[#1A1A1A] dark:text-[#F4F2ED]">
              {profile.name[language]}
            </h2>
            <span className="inline-flex max-w-full items-center gap-1.5 bg-[#EBE5DE] dark:bg-[#1E1D1B] px-3 py-1 text-[10px] font-mono tracking-widest text-[#6C6863] dark:text-[#A39E98] uppercase border border-[#1A1A1A]/10 dark:border-white/15">
